...
const MainPage = () => {
  ...
  const [pickDate, pickDateValue] = React.useState(`${new Date().getFullYear()}-${new Date().getMonth() + 1}-${new Date().getDate()}`);
  ...
  const contents = {
    0: <NowLocation myParams={myParams_real_time} />,
    1: <AllWork myParams={myParams} />,
    2: <RouteAnalysis myParams={myParams} />,
    3: <Chatting />,
    4: <Testpage myParams={myParams} />,
  }
  const menu_header = {
    0: '실시간 업무 현황',
    1: '업무 내역',
